...
raw_data <- read_excel("output_files/data/2a_perseus_data.xlsx")
```
...
#missforest on event_data
#new <- event_data %>% 
#  dplyr::select(-c(Healing_time, Healing, avg_TBI, ID_Nr, event_all)) %>% 
#  as.data.frame()

#set.seed(123)
#RF_data_imputed <- missForest(xmis=new,verbose = TRUE, xtrue = new)
#OOB <- RF_data_imputed$OOBerror
#event_data <- RF_data_imputed$ximp
...
```{r boxplot for healing time}
plot <- ggplot(event_data,
       aes(x = as.factor(Healing_time),
           y = Days_to_heal_cox,
           fill = as.factor(Healing_time))) +
       geom_boxplot() +
       xlab("Healing category") + 
  ylab("Healing time") +
       labs(title = paste("Boxplots for simulated healing time categorized by Healing type")) + 
       guides(fill = "none")

ggsave(filename = paste0("output_files/figures/1a/2_boxplot_healing_simulated.png"), 
  plot,
  width = 10, 
  height = 7, 
  device='tiff', 
...
```{r barplot}
event_data$subject <- paste0("S",seq(nrow(event_data)))
event_data$status <- ifelse(event_data$event_all==1,"Event","Censored event")

plot <- ggplot(event_data, aes(subject, 
                               Days_to_heal_cox)) +
  geom_bar(stat = "identity", 
           width = 0.5, 
           position = position_dodge(0.9),
           aes(fill=as.factor(Healing_time))) +
  geom_point(data = event_data, 
             aes(subject, 
                 Days_to_heal_cox, 
                 color = as.factor(Healing_time), 
                 shape = status), 
             size = 2)  +
  coord_flip() +
  theme_bw() + 
  ...
  geom_text(aes(label = "180 days", 
            ...
            color = "black") +
  xlab("Subjects") + 
  ylab("Time to heal (days)") +
  scale_fill_discrete(labels = c("Fast-healing", "Slow-healing", "Chronic")) +
  guides(color = F)
         
if(save.plots) ggsave(plot, 
                      filename = "output_files/figures/1a/time_barplot.png",
                      width = 10, 
                      height = 7, 
                      device='tiff', 
                      dpi=700)
...
```{r ignore}
coxph(SurvObj~CON__P00761+Age+Gender+Triglyceride+Diabetes_Type+Diabetes_Duration+HBA1C+BMI, data = event_data)

#coef — estimated hazard coefficient on the log scale
#exp(coef) — the hazard ratio (HR). Exponent of coef
#Here, 0.97 means 97%. So, CON__P00761 reduces the hazard by a factor of 3%,
# and age increases the hazard by a factor of 2.1%

coxph(SurvObj~CON__P00761+Age+Gender+Triglyceride+Diabetes_Type+Diabetes_Duration+HBA1C+BMI, data = event_data) %>% tidy(conf.int = T)
```
...
if(save.data) write.xlsx(as.data.frame(results_cox_adjusted), 
                         "output_files/data/2b_cox_adjusted.xlsx", 
                         row.names = FALSE)
...
uniprot_names <- results_cox_filtered %>% 
  ...
  dplyr::select(c(Gene.name, UniProt.names, estimate, std.error, statistic, p.value, p.value.adjusted, conf.low, conf.high)) %>% 
  mutate(HR = exp(estimate))

if(save.data) write.xlsx(as.data.frame(uniprot_names), 
                         "output_files/data/2b_cox_adjusted_uniprotnames.xlsx", 
                         row.names = FALSE)
...
```{r corrplot}
require("corrplot")
corplot_list <- results_cox_filtered$term
corplot_gene_names <- str_extract(results_cox_filtered$Gene.name,"[^;]+")

corplot_data <- raw_data[, corplot_list] %>% 
  set_names(corplot_gene_names)

M<-cor(corplot_data, method = "spearman")
#corrplot(M, method="color",  order="hclust")
png("output_files/figures/2b/2b_corr_proteins.png", width=7.5, height=7.5, units = "in", res = 300)
gplots::heatmap.2(M,
                  revC = TRUE,
                  trace = "none",
                  density.info = "none",
...
```
```{r}
covariates <- c("Age","Gender", "Triglyceride", "Diabetes_Type", "Diabetes_Duration", "HBA1C", "BMI")
corplot_data <- raw_data[, corplot_list] %>% 
  set_names(corplot_gene_names) %>% 
  cbind(raw_data[, covariates])
  
M<-cor(corplot_data, use='pairwise.complete.obs', method = "spearman")


M_fin <- M[covariates,corplot_gene_names]

png("output_files/figures/2b/2b_corr_covariates.png", width=7.5, height=7.5, units = "in", res = 300)
gplots::heatmap.2(M_fin, #Colv = NA, 
                  Rowv = NA, 
                  revC = TRUE,
    trace = "none", 
    density.info = "none",
  ...
  theme(plot.title = element_text(size = rel(1.5), 
    ...
    geom_text_repel(data=results_cox_filtered,
                    aes(x = exp(estimate), 
                        ...
                        label=str_extract(Gene.name,"[^;]+"),  # (UniProt.names,"[^_]+")
...
if(save.plots) ggsave(plot, 
                      filename = "output_files/figures/2b/volcano_plot.png",
                      width = 10, 
                      height = 7, 
                      device='tiff', 
                      dpi=100)
ggsave(plot,
       filename = "output_files/figures/2b/2b_volcano_plot.png",
       width = 7.5, 
       height = 4.5)
```
...
if(save.data) sink("output_files/figures/2b/fit_summary_all.txt")
...
if(save.plots) ggsave(plot, 
                      filename = "output_files/figures/2b/KM_curve_all.png",
                      width = 7.5,
                      height = 7.5)
...
if(save.data) sink("output_files/figures/2b/fit_summary_Dtype.txt")
print(summary(fit, times = c(1,5*(1:40))))
cat("\n")
...
if(save.plots) ggsave(file = "output_files/figures/2b/KM_curve_Dtype.png",
                      plot,
                      width = 7.5,
                      height = 7.5)
...
if(save.data) sink("output_files/figures/2b/fit_summary_gender.txt")
print(summary(fit, times = c(1,5*(1:40))))
cat("\n")
...
if(save.plots) ggsave(file = "output_files/figures/2b/KM_curve_gender.png",
                      plot,
                      width = 7.5,
                      height = 7.5)
```
...
if(save.data) sink("output_files/figures/2b/fit_summary_Dduration.txt")
...
if(save.plots)ggsave(file = "output_files/figures/2b/KM_curve_Dduration.png",
                      plot,
                      width = 7.5,
                      height = 4.5)
...
if(save.data) sink("output_files/figures/2b/fit_summary_bmi.txt")
...
if(save.plots) ggsave(file = "output_files/figures/2b/KM_curve_bmi.png",
                      plot,
                      width = 7.5,
                      height = 4.5)
...
if(save.data) sink("output_files/figures/2b/fit_summary_age.txt")
...
if(save.plots) ggsave(file = "output_files/figures/2b/KM_curve_age.png",
                      plot,
                     width = 7.5,
                      height = 4.5)
...
if(save.data) sink("output_files/figures/2b/fit_summary_HBA1C.txt")
...
if(save.plots) ggsave(file = "output_files/figures/2b/KM_curve_HBA1C.png",
                      plot,
                      width = 7.5,
                      height = 4.5)
...
if(save.data) sink("output_files/figures/2b/fit_summary_triglycerides.txt")
...
if(save.plots) ggsave(file = "output_files/figures/2b/KM_curve_triglycerides.png",
                      plot,
                      width = 7.5,
                      height = 4.5)
```
...
for (i in 1:nrow(results_cox_filtered)) {
  ...
  if(save.data) sink(paste0("output_files/figures/2b/fit_summary_", 
                                      str_extract(results_cox_filtered$Gene.name[i],"[^;]+"), ".txt"))
  print(summary(fit))
  sink()
  ...
  plot <- plot_surv_contour(time="Days_to_heal_cox",
               ...
               title=paste0("Survival Probability Contour Curve for DFU Healing [Protein -> ", 
                            str_extract(results_cox_filtered$Gene.name[i],"[^;]+"), "]"),
               legend.title="Probability of not healing",
               xlab = "Time (days)",
               ylab = str_extract(results_cox_filtered$Gene.name[i],"[^;]+")) + 
  annotate("text", x = -Inf, y = -Inf,
           vjust = -2, hjust = -1, 
           label = bquote("P"[FDR]*"="~ .(pvalue)),
           colour = "white")
  
  if(save.plots) ggsave(file = paste0("output_files/figures/2b/2b_KM_curve_", 
                                      str_extract(results_cox_filtered$Gene.name[i],"[^;]+"), ".png"),
                                      plot,
                                      width = 7.5,
                        height = 4.5)
