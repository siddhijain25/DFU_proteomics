...
cox_filter <- read_excel("output_files/data/2b_cox_adjusted.xlsx")
...
raw_data_encoded <- dummyVars(" ~ .",
                              data = raw_data[, columns_to_onehot], 
                              keepUnusedLevels = TRUE)

raw_data_encoded <- data.frame(predict(raw_data_encoded, 
                                       raw_data[, columns_to_onehot])) %>% 
...
```{r filtering significant proteins}
...
RF_data <- raw_data_final %>% 
  dplyr::select(one_of(cox_signif)) 
...
grid <- expand.grid(mtry = seq(from = 1, to = 15, by = 1) )
```
...
grid <- expand.grid(mtry = seq(from = 1, to = 265, by = 10) )
```
